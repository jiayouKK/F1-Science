(function (global) {
  ...
  // Screw gauge draggable simulator — verified: sleeve = floor(total/0.5)*0.5, thimble = round((total-sleeve)*100)
  function meter(mount, opts) {
    ...
    function draw(hundredths) {
      ...
      readout.innerHTML = `Sleeve <strong>${sleeve.toFixed(2)} mm</strong> + Thimble <strong>${thimble} × 0.01</strong> = <strong>${total.toFixed(2)} mm</strong>`;
      ...
      svgWrap.innerHTML = `
        <svg width="100%" viewBox="0 0 420 150" role="img">
          ...
          <text x="${sleeveEnd + 18}" y="78" font-size="16" fill="#d43b2e" font-weight="700">${thimble}</text>
          <text x="${sleeveEnd + 44}" y="95" font-size="8" text-anchor="middle" fill="#6b6a63">× 0.01 mm</text>
        </svg>
      `;
    }
  ...
  // Vernier caliper draggable simulator — verified: main = floor(h/10)/10, vernierDigit = h%10, exact reconstruction
  function vernier(mount, opts) {
    const root = el(shell(opts.title || 'Drag the sliding jaw'));
    const body = root.querySelector('.game-body');
    mount.appendChild(root);
    body.innerHTML = `
      <p class="game-prompt">Drag to move the sliding jaw. Main scale reads to 0.1 cm; the highlighted vernier line adds the last 0.01 cm.</p>
      <div class="meter-svg"></div>
      <input class="meter-range" type="range" min="0" max="500" value="322" step="1" aria-label="Vernier caliper opening">
      <div class="meter-readout"></div>
      ${takeaway(opts.takeaway)}
    `;
    const svgWrap = body.querySelector('.meter-svg');
    ...
    function draw(hundredths) {
      const main = Math.floor(hundredths / 10) / 10;
      const vernierDigit = hundredths % 10;
      const total = main + vernierDigit * 0.01;
      readout.innerHTML = `Main <strong>${main.toFixed(1)} cm</strong> + Vernier <strong>${vernierDigit} × 0.01</strong> = <strong>${total.toFixed(2)} cm</strong>`;

      const pxPerCm = 60;
      const pointerX = 30 + main * pxPerCm;
      const vernierBoxX = pointerX;

      // Vernier scale strip: 10 ticks (0-9), highlight the coinciding one
      let vernierTicks = '';
      for (let d = 0; d <= 9; d++) {
        const x = 6 + d * 9;
        const isHit = d === vernierDigit;
        vernierTicks += `<line x1="${x}" y1="0" x2="${x}" y2="${isHit ? 12 : 7}" stroke="${isHit ? '#d43b2e' : '#2a5fd4'}" stroke-width="${isHit ? 1.6 : 0.7}"/>`;
        if (isHit) vernierTicks += `<text x="${x}" y="24" font-size="8" text-anchor="middle" fill="#d43b2e" font-weight="700">${d}</text>`;
      }

      svgWrap.innerHTML = `
        <svg width="100%" viewBox="0 0 400 130" role="img">
          <title>Interactive vernier caliper</title>
          <rect x="20" y="55" width="330" height="12" fill="#e8e5da" stroke="#3d3d3a" stroke-width="1.2"/>
          <path d="M32 55 L32 25 L44 25 L44 55" fill="#e8e5da" stroke="#3d3d3a" stroke-width="1.2"/>
          <g stroke="#3d3d3a" stroke-width="0.6">
            ${[0,1,2,3,4,5].map(n => {
              const x = 30 + n * pxPerCm;
              return `<line x1="${x}" y1="55" x2="${x}" y2="60"/><text x="${x}" y="50" font-size="9" text-anchor="middle" fill="#6b6a63">${n}</text>`;
            }).join('')}
          </g>
          <line x1="${pointerX}" y1="67" x2="${pointerX}" y2="76" stroke="#d43b2e" stroke-width="1.5"/>
          <polygon points="${pointerX},76 ${pointerX-4},82 ${pointerX+4},82" fill="#d43b2e"/>

          <g transform="translate(${vernierBoxX - 6}, 90)">
            <rect x="-2" y="-4" width="94" height="30" fill="#fff" stroke="#3d3d3a" stroke-width="1"/>
            ${vernierTicks}
          </g>
          <text x="${vernierBoxX + 40}" y="86" font-size="9" text-anchor="middle" fill="#6b6a63">vernier scale (slides with jaw)</text>
        </svg>
      `;
    }
  ...
  global.Games = { ghsIcon, match, order, meter, vernier, attachProgress };
})(window);
